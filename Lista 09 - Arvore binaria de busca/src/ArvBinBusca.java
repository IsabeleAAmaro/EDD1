...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> {
    ...
    //Questão 02
    //Complexidade: O(n) sendo n número de elementos do vetor
    ...
    //Questão 03:
   //Ver complexidade depois

    public void transformaBST(Arvbin<Chave> arvoreBinaria) {
        //O QUE É ESSE VALOR??
        Valor valor = (Valor) Integer.valueOf(0);
        ...
        this.put(arvoreBinaria.retornaVal(), valor);
    ...
    private void removeForaIntervalo(No no, Chave chaveMin, Chave chaveMax ) {
        if (no.esq != null)
            removeForaIntervalo(no.esq, chaveMin, chaveMax);

        if (no.dir != null)
            removeForaIntervalo(no.dir, chaveMin, chaveMax);
    ...
    public Chave obtemAncestralComum(Chave primeiraChave, Chave segundaChave) {
        if (get(primeiraChave) == null || get(segundaChave) == null)
            throw new NoSuchElementException("Chaves incorretas ou não existem.");

        No pai1 = retornaPai(primeiraChave);
        No pai2 = retornaPai(segundaChave);

        while (pai1 != null || pai2 != null) {
            if (pai1 == pai2)
                break;

            if (altura(pai1) < altura(pai2))
                pai1 = retornaPai(pai1.chave);

            else if (altura(pai1) > altura(pai2))
                pai2 = retornaPai(pai2.chave);
    ...
    public No retornaPai(Chave chave) {
        return retornaPai(raiz, chave);
    }

    public No retornaPai(No no, Chave chave) {
        No pai = null;

        if ((no.esq != null && no.esq.chave == chave) || (no.dir != null && no.dir.chave == chave))
            pai = no;

        if (no.esq != null && pai == null)
            pai = retornaPai(no.esq, chave);

        if (no.dir != null && pai == null)
            pai = retornaPai(no.dir, chave);
