...
public class Arvbin<T extends Comparable<T>> {
	...
	public int[] armazenaArvbinVetor(Arvbin<Integer> arvore) {
		int tamanho = arvore.contaNos();
		int[] vetor = new int[tamanho];

		armazenaVetorRecursivo(arvore, vetor, 0, 1);

		return vetor;
	}

	private int armazenaVetorRecursivo(Arvbin<Integer> arvore, int[] vetor, int indice, int pos) {
		if (arvore != null) {
			vetor[indice] = arvore.retornaVal();
			indice++;

			indice = armazenaVetorRecursivo(arvore.retornaEsq(), vetor, indice, 2 * pos);
			indice = armazenaVetorRecursivo(arvore.retornaDir(), vetor, indice, 2 * pos + 1);
		}
